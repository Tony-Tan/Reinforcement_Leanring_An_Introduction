...
class Agent:
    def __init__(self, environment_):
        ...
        self.tiling_block_num = 32
        self.tiling_num = 4
        self.size_of_weights = self.tiling_num * self.tiling_block_num * self.tiling_block_num
        self.value_of_state_action = LinearFunction(self.size_of_weights + 3)
    ...
    def select_action(self, state_feature, epsilon=0.1):
        ...
        for action_iter in range(self.env.action_space.n):
            state_action_feature = self.active_features(state_feature, action_iter)
    ...
    def running(self, iteration_times, alpha=0.01, gamma=0.9, lambda_coe=0.9):
        ...
        for iteration_time in range(iteration_times):
            ...
            step_num = 0
            eligibility_trace = np.zeros(self.size_of_weights + 3)
            ...
            while True:
                ...
                delta = reward
                state_action_feature = self.active_features(state_feature, action)
                ...
                next_active_features = self.active_features(next_state_feature, next_action)
                ...
                action = next_action
                # self.select_action(state_feature)
...
if __name__ == '__main__':
    ...
    for _ in range(repeat_times):
        print('1 round ' + str(_))
        agent = Agent(env)
        step_num_list += agent.running(50, alpha=0.001, lambda_coe=0.99)
