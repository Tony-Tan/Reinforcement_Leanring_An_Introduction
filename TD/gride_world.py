...
class GridWorld:
    ...
    def plot_grid_world(self, policy=collections.defaultdict(float)):
        n = self.grid_size
            ...
            for j in range(n):
                ...
                for action_iter in range(size_of_action_space):
                    if policy[i*n+j][action_iter] != 0:
                        ...
                        if action_iter == 0:
                            plt.arrow((j + 1) / (n + 1), 1 - (i + 1) / (n + 1),  -1*length/ (4 * (n + 1)),
                                  -0, head_width=0.01, head_length=0.01, fc='k',
                                  ec='k')
                        if action_iter == 1:
                            plt.arrow((j + 1) / (n + 1), 1 - (i + 1) / (n + 1),  0,
                                  +1*length/ (4 * (n + 1)), head_width=0.01, head_length=0.01, fc='k',
                                  ec='k')
                        if action_iter == 2:
                            plt.arrow((j + 1) / (n + 1), 1 - (i + 1) / (n + 1),  +1*length/ (4 * (n + 1)),
                                  -0, head_width=0.01, head_length=0.01, fc='k',
